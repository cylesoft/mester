...
function identity_step() {
	fs.readFile(identity_filename, { "encoding": "utf8" }, function(err, data) {
		if (err) {
			alert('No identity file present, creating one for you!');
			this_username = prompt('Your username?');
			alert('Saving username, UUID, and generating public/private keypair.');
			alert('This may take a few seconds...');
			this_username = this_username.trim();
			this_uuid = uuid.v4();
